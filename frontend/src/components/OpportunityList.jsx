...
function OpportunityList() {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      const fetchOpportunities = async () => {
        ...
        try {
          setLoading(true);
          const url = `${API_BASE_URL}/api/opportunities?search=${searchTerm}&page=${currentPage}&limit=10`;
  ...
  const handleViewFiles = async opportunityId => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/files/Opportunity/${opportunityId}`,
        {
          headers: { 'x-auth-token': token },
        }
      );
  ...
  const handleViewActivity = async opportunityId => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/activities/Opportunity/${opportunityId}`,
        {
          headers: { 'x-auth-token': token },
        }
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/opportunities/${idToDelete}`,
        {
          method: 'DELETE',
          headers: { 'x-auth-token': token },
        }
    ...
    <div>
      ...
      {editingOpportunity && (
        <OpportunityEditForm
          ...
          onUpdateSuccess={handleOpportunityUpdated}
          onCancel={handleCancelForm}
        />
      )}
                  ...
                  <tr>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => handleViewActivity(opportunity._id)}
                      ...
                      <button
                        ...
                        onClick={() => handleViewFiles(opportunity._id)}
                        style={{ marginRight: '8px' }}>
                      ...
                      <button
                        ...
                        onClick={() => openDeleteModal(opportunity._id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
